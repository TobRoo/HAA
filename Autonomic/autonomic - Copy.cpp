...
int AgentBase::_receiveAtomicMessage( DataStream *ds ) {
	...
	int oldOrder; // highest order for this message we were aware of
	...
	if ( this->atomicMsgs.find( id ) != this->atomicMsgs.end() ) {
		...
		newMsg = false;
		oldOrder = this->atomicMsgs[id].order;
	}
...
bool AgentBase::cbDelayedMessage( void *vpDelayedMessage ) {
	...
	case DMT_NORMAL:
		...
		if ( iC != this->connection.end() ) { // connection is still good
			this->sendMessage( iC->second, dm->message, (char *)getDynamicBuffer(dm->bufRef), dm->len, 
				( dm->forwardAddress == nil ? NULL : &dm->forwardAddress ),
				( dm->returnAddress == nil ? NULL : &dm->returnAddress ) );
		}
		break;
	case DMT_GROUP:
	...
	case DMT_EX:
		iC = this->connection.find( dm->conId );
		if ( iC != this->connection.end() ) { // connection is still good
			this->sendMessageEx( iC->second, dm->message, dm->msg_size, dm->msg_first, dm->msg_last, (char *)getDynamicBuffer(dm->bufRef), dm->len,
				( dm->forwardAddress == nil ? NULL : &dm->forwardAddress ),
				( dm->returnAddress == nil ? NULL : &dm->returnAddress ) );
		}
		break;
