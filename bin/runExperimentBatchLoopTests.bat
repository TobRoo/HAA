...
:WATCHDOG
set number=0
for /f "skip=3" %%x in ('tasklist /FI "IMAGENAME eq Autonomic.exe"') do set /a number=number+1
echo Total Autonomic.exe tasks running: %number%
if %number% geq 3 (
goto RUNNING
)
echo Not running...
...
tasklist /nh /fi "imagename eq Autonomic.exe" /fi "STATUS eq NOT RESPONDING" | find /i "Autonomic.exe" >nul && (
goto CRASHED
)
